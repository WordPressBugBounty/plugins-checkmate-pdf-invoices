...
class Style
{
    ...
    public function fromStyleSheets(AbstractTag $tag, $attributes) {
        ...
        $outputFormat = \Checkmate\Vendor\Sabberworm\CSS\OutputFormat::createCompact();

        $styles = array();

        foreach ($stylesheets as $_sc) {

            /** @var \Checkmate\Vendor\Sabberworm\CSS\RuleSet\DeclarationBlock $_decl */
            foreach ($_sc->getAllDeclarationBlocks() as $_decl) {

                /** @var \Checkmate\Vendor\Sabberworm\CSS\Property\Selector $_selector */
                foreach ($_decl->getSelectors() as $_selector) {
                    ...
                    if ($class !== null) {
                        foreach ($class as $_class) {
                            if ($_selector === ".$_class") {
                                /** @var \Checkmate\Vendor\Sabberworm\CSS\Rule\Rule $_rule */
                                foreach ($_decl->getRules() as $_rule) {
                                    $value = $_rule->getValue();
                                    if ($value instanceof \Checkmate\Vendor\Sabberworm\CSS\Value\Value) {
